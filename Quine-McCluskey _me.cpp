  ...
  for(int i = 0 ; i < variable_num + 1 ; i++){
    ...
    for(int j = 0 ; j < all_mintern_num ; j++){
      if(mintern_bin[j][variable_num] == i){
        ...
        cout<<"check min to col:"<< column[0][i][k][0] <<";"<<j<<endl;
        k++;
      }
    }
  }
        ...
        if(column[i][j][k] != NULL && column[i][j][k][variable_num+1] == 0){
          ...
          if(bool_record != 0){
            ...
            for(int l = 0 ; l < pow(2 , i) ; l++){
              ...
              cout<<"check inside this loop:"<<l<<";pow: "<<pow(2 , i)<<";col: "<<column[i][j][k][variable_num+3+i+l]<<endl;
              cout<<"check counter:"<<counter[column[i][j][k][variable_num+3+i+l]]<<endl;
